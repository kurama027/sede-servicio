node {
   ...
   stage('Preparation') {
       git 'https://github.com/kurama027/sede-servicio.git'
       mvnHome = tool 'MAVEN'
   }
   ...
   stage('Deployment') {
       try {
			sh 'chown jenkins:jenkins /var/lib/jenkins/workspace/helena-servicio-sede/runDeployment.sh'
			sh 'chmod +x /var/lib/jenkins/workspace/helena-servicio-sede/runDeployment.sh'
           sh '/var/lib/jenkins/workspace/helena-servicio-sede/runDeployment.sh'
       }catch (e){
